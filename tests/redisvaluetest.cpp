...
BOOST_AUTO_TEST_CASE(test_integer)
{
    ...
    BOOST_CHECK_EQUAL(v.toInt64(), 100);
    BOOST_CHECK_EQUAL(v.isInt(), true);
    BOOST_CHECK_EQUAL(v.isOk(), true);
}

BOOST_AUTO_TEST_CASE(test_integer_positive)
{
    RedisValue v(std::numeric_limits<int64_t>::max());

    BOOST_CHECK_EQUAL(v.toInt64(), std::numeric_limits<int64_t>::max());
    BOOST_CHECK_EQUAL(v.isInt(), true);
    BOOST_CHECK_EQUAL(v.isOk(), true);
}
...
BOOST_AUTO_TEST_CASE(test_integer_negative)
{
    ...
    BOOST_CHECK_EQUAL(v.toInt64(), std::numeric_limits<int64_t>::min());
    BOOST_CHECK_EQUAL(v.isInt(), true);
    BOOST_CHECK_EQUAL(v.isOk(), true);
}
