...
namespace
{
    ...
    inline void bufferAppend(std::vector<char> &vec, const redisclient::RedisBuffer &buffer)
    {
        if (buffer.data.type() == typeid(std::string))
            bufferAppend(vec, boost::get<std::string>(buffer.data));
        else
            bufferAppend(vec, boost::get<std::vector<char>>(buffer.data));
    }
    ...
    /* Socket function */
    size_t socketReadSomeImpl(int socket, char *buffer, size_t size, size_t timeout)
    {
        struct timeval tv = {static_cast<time_t>(timeout / 1000),
                             static_cast<__suseconds_t>((timeout % 1000) * 1000)};
        ...
        boost::system::error_code &ec)
    {
        ...
        for (;;)
        {
            ssize_t result = socketReadSomeImpl(socket, boost::asio::buffer_cast<char *>(buffer) + bytesRecv, boost::asio::buffer_size(buffer) - bytesRecv, timeoutMsec);
    ...
    size_t socketWrite(
        int socket, boost::asio::const_buffer buffer,
        const boost::posix_time::time_duration &timeout,
        boost::system::error_code &ec)
    {
        size_t bytesSend = 0;
...
{
    RedisClientImpl::RedisClientImpl(boost::asio::io_service &ioService_t)
        : ioService(ioService_t),
          strand(ioService),
          socket(ioService),
          bufSize(0),
          subscribeSeq(0),
    ...
    void RedisClientImpl::asyncWrite(const boost::system::error_code &ec, size_t)
    {
        ...
        if (dataQueued.empty() == false)
        {
            ...
            std::swap(dataQueued, dataWritten);
    ...
    void RedisClientImpl::handleAsyncConnect(
        const boost::error_code &ec,
        std::function<void(boost::system::error_code)> handler)
    {
        if (!ec)
        {
            boost::system::error_code ec2 = ec2;
    ...
    RedisValue RedisClientImpl::doSyncCommand(
        ...
        boost::system::error_code &ec)
    {
        for (;;)
        {
            ...
            for (size_t pos = 0; pos < bufSize;)
            {
                ...
                else if (result.second == RedisParser::Uncompleted)
                {
                    continue;
                }
                else
    ...
    void RedisClientImpl::doSyncCommand(std::vector<char> buff, std::function<void(RedisValue)> handler)
    {
        handlers.push(std::move(handler));
        dataQueued.push_back(std::move(buff));

        if (dataWritten.empty())
        {
    ...
    void RedisClientImpl::defaultErrorHandler(const std::string &s)
    {
        ...
        std::function<void(RedisValue)> handler)
    {
        ...
        else
        {
            ...
            ss << "RedisClientImpl::subscribe called with invalid state " << to_string(state);
        ...
        std::function<void(RedisValue)> handler)
    {
        ...
        else
        {
            ...
            ss << "RedisClientImpl::singleShotSubscribe called with invalid state " << to_string(state);
        ...
        std::function<void(RedisValue)> handler)
    {
        ...
        if (state == State::Connected || state == State::Subscribed)
        {
            ...
            post(std::bind(&RedisClientImpl::doAsyncCommand, this, makeCommand(items), handler));
        }
        else
        {
            std::stringstream ss;

            ss << "RedisClientImpl::unsubscribe called with invalid state " << to_string(state);
